...
// shiny.js
var jsTreeSelectorInputBinding = new Shiny.InputBinding();


// An input binding must implement these methods
$.extend(jsTreeSelectorInputBinding, {

  // This returns a jQuery object with the DOM element
  find: function(scope) {
    return $(scope).find('div[type="jsTreeSelectorInput"]');
  },
  ...
  subscribe: function(el, callback) {
    ...
    $(el).on('keyup.jsTreeSelectorInputBinding input.jsTreeSelectorInputBinding', function(event) {
      callback(true);
      // When called with true, it will use the rate policy,
      // which in this case is to debounce at 500ms.
    });
    $(el).on('change.jsTreeSelectorInputBinding', function(event) {
      callback(false);
  ...
  unsubscribe: function(el) {
    $(el).off('.jsTreeSelectorInputBinding');
  },
...
Shiny.inputBindings.register(jsTreeSelectorInputBinding, 'shiny.jsTreeSelectorInput');

$(document).on("click","div[type='jsTreeSelectorInput']",function(evt){
  var el = $(evt.target);
  //console.log('clicked');
  //console.log(el);
  el.jstree(true).hide_icons();
...
var initjsTreeSelectorInput = function (inputID,message,selected) {
